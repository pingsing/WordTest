...
public class WordQuiz {
		...
		private int makeExample(int ex[], int answerIndex) {		// 보기 구성
			int n[] = {-1, -1, -1, -1};		// 보기를 출력하기위한 index 번호로 활용
			int index;
			for(int i=0; i<n.length; i++) {
				do {
					index = (int)(Math.random()* w.size());	//0~16
				}while(index == answerIndex || exists(n, index));	
					// 정답과 같은 index 번호이거나 n배열이 이미 들어있는 값이면 사용불가 판정
					n[i] = index;
			...
			return (int)(Math.random() * n.length);		// 0 ~ 3의 난수 ex배열의 정답이 들어갈 위치값.
		} // makeExample end
